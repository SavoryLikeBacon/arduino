...
double brightness[] = {100, 30, 15, 8, 2};
...
void toe_hit(){
    send_ball( 0, 21, 43, 22, 8); // in pulse
...
void heel_hit(){
    send_ball( 22, 43, 21, 0, 8); // out pulse
...
void send_ball(int start_bit0, int end_bit0, int start_bit1, int end_bit1, int wait){  // send ball backward (toe step)
    ...
    for(int t = start_bit0; t < end_bit0+6; t++){   
        if (i <= end_bit0)
        ...
        if(i <= end_bit0+5)
          strip.setPixelColor( i-5, 0 );  // clear bit after tail
        ...
        if(i2 >= end_bit1-5)
          strip.setPixelColor( i2+5, 0 );  // clear bit after tail

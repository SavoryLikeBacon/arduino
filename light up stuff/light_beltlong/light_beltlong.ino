...
void setup() {
  ...
  //rainbowCycle(0);
  //battery_check();
  colorWipe(strip.Color(4,4,0), 20);
  theaterChase(strip.Color(4, 4, 0), 50);
  colorWipe(0, 0);
...
void toe_hit(){
    send_ball( 0, 119, 239, 120, 6); // in pulse
    
    strip.setPixelColor(120,strip.Color(0, 0, 0));
    strip.setPixelColor(119,strip.Color(0, 0, 0));
     strip.show();
...
void heel_hit(){
    send_ball( 120, 240, 119, 0, 6); // out pulse
...
void back_and_forth(){
    send_ball( 120, 240, 119, 0, 30); // out pulse
    //Serial.print("heel hit | ");
    digitalWrite(ledPin, HIGH);
    
    send_ball( 0, 119, 239, 120, 30); // in pulse
    //Serial.print("toe hit | ");
    digitalWrite(ledPin, LOW);   
}

...
const int toe_pin = A0;  // toe pressure sensor
...
double brightness[] = {100, 50, 25, 18, 10,7,5,3,2,1};
...
void back_and_forth(){
    int cycle_speed = 14;
    send_ball( 30, 60, 29, 0, cycle_speed); // out pulse
    //Serial.print("heel hit | ");
    digitalWrite(ledPin, HIGH);
    
    send_ball( 0, 29, 59, 30, cycle_speed); // in pulse
    //Serial.print("toe hit | ");
    digitalWrite(ledPin, LOW);   
}
...
void send_ball(int start_bit0, int end_bit0, int start_bit1, int end_bit1, int wait){  // send ball backward (toe step)
    ...
    for(int t = start_bit0; t < end_bit0+7; t++){   
        ...
        if (i1 <= end_bit0)
          strip.setPixelColor(i1,strip.Color(100, 100, 0));  // set initial ball   
       
       if(i2 >= end_bit1)
          strip.setPixelColor(i2,strip.Color(100, 0 ,100));  // set initial ball
       int tails=9;  
       for (int i0=1;i0<tails;i0++){
          if(i1 <= end_bit0+i0)
            strip.setPixelColor( i1-i0, brightness[i0] );  // set first tail
            
          if(i2 >= end_bit1-i0)
             strip.setPixelColor( i2+i0, brightness[i0] );  // set first tail
          
        } 
        if(i1 <= end_bit0+(tails+1))
          strip.setPixelColor( i1-(tails+1), 0 );  // clear bit after tail 
        if(i2 >= end_bit1-(tails+1))
          strip.setPixelColor( i2+(tails+1), 0 );  // clear bit after tail
      
      /*  
        if(i1 <= end_bit0+1)
          strip.setPixelColor( i1-1, brightness[1] );  // set first tail
        if(i1 <= end_bit0+2)
          strip.setPixelColor( i1-2, brightness[2] );  // set second tail
        if(i1 <= end_bit0+3)
          strip.setPixelColor( i1-3, brightness[3] );  // set third tail
        if(i1 <= end_bit0+4)
          strip.setPixelColor( i1-4, brightness[4] );  // set fourth tail    
        if(i1 <= end_bit0+5)
          strip.setPixelColor( i1-5, brightness[5] );  // set fourth tail  
        if(i1 <= end_bit0+6)
          strip.setPixelColor( i1-6, brightness[6] );  // set fourth tail  
        if(i1 <= end_bit0+7)
          strip.setPixelColor( i1-7, 0 );  // clear bit after tail
